        ...
        return (
            <div className={'demo-step'}>
                ...
                <DemoRow title={'步骤状态'}>
                    <DemoLine>
                        <PlStepGroup dotIcon>
                            <PlStep title="Finish" content="This step is finish" status="finish"/>
                            <PlStep title="Process" content="This step is processing..." status="process"/>
                            <PlStep title="error" content="This step is error!" status="error"/>
                            <PlStep title="Wait" content="This step is not start." status="wait"/>
                        </PlStepGroup>
                    </DemoLine>
                    <DemoLine>
                        <PlStepGroup>
                            <PlStep title="Finish" content="This step is finish" status="finish" icon="el-icon-success"/>
                            <PlStep title="Process" content="This step is processing..." status="process" icon="el-icon-s-tools"/>
                            <PlStep title="error" content="This step is error!" status="error" icon="el-icon-error"/>
                            <PlStep title="Wait" content="This step is not start." status="wait" icon="el-icon-info"/>
                        </PlStepGroup>
                    </DemoLine>
                    <DemoLine>
                ...
                <DemoRow title={'通过绑定val切换步骤'}>
                    ...
                    <PlStepGroup current={state.val1[3]} currentStatus={state.val1[3] === 'step5' ? 'finish' : undefined}>
                        <PlStep title="获取token" content="调用接口，获取token" val={'step1'}/>
                        <PlStep title="上传logo" content="使用token上传logo图片" val={'step2'}/>
                        <PlStep title="创建卡券" content="调用接口创建卡券信息" val={'step3'}/>
                        <PlStep title="创建二维码" content="调用接口创建二维码" val={'step4'}/>
                        <PlStep title="显示二维码" content="在应用中显示二维码" val={'step5'}/>
                    </PlStepGroup>
                    <div style={{
                        height: '100px',
                        display: 'flex',
