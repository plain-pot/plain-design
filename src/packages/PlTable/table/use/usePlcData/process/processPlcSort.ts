...
function getPlcOrder(item: tPlcType): number {
    ...
    if (item.props.fixed === ePlcFixedType.left) {
        order -= 9999
    } else if (item.props.fixed === ePlcFixedType.right) {
        order += 9999
    }
    return Number(order)
}
...
export function processPlcSort(
    ...
    const {targetTableWidth, plcKeyString} = (() => {
        ...
        flatPlcList.forEach(plc => {
            ...
            if (plc.props.fit) {
                fitPlc = plc
            } else if ((!fitPlc || !fitPlc.props.fit) && plc.props.fixed === ePlcFixedType.center) {
                fitPlc = plc
            }
