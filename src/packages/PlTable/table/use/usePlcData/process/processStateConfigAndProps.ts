...
export function processStateConfigAndProps(
    ...
    iteratePlcList({
        ...
        onPlc: plc => {
            ...
            if (plc.props.fixed === ePlcFixedType.right) configState.fixedRight.push(plc)
    ...
    if (configState.fixedLeft.length > 0) {configState.autoFixedLeft.forEach(plc => plc.props.fixed = ePlcFixedType.left)}

    /*如果存在右固定列，则设置autoFixedRight为右固定*/
    if (configState.fixedRight.length > 0) {configState.autoFixedRight.forEach(plc => plc.props.fixed = ePlcFixedType.right)}
