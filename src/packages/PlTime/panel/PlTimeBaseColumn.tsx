...
export const PlTimeBaseColumn = designComponent({
    ...
    setup({props, event}) {
        ...
        return {
            render: () => (
                <div className="pl-time-base-column">
                    <PlScroll ref={onRef.scroll} onScroll={handler.onScroll}>
                        <ul className="pl-time-base-column-list">
                            {[1, 2, 3].map((item, index) => (
                                <li
                                    ref={val => liList[index] = val as any}
                                    className="pl-time-base-column-item"
                                    key={-item}/>)
                            )}
                            ...
                            {[1, 2, 3].map(item => (<li className="pl-time-base-column-item" key={-item - 3}/>))}
                        </ul>
                    </PlScroll>
                </div>
            )
