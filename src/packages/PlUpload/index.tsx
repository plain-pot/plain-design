...
export const PlUpload = designComponent({
    ...
    setup({props, slots, scopeSlots, event: {emit}}) {
        ...
        const renderItem = (file: UploadFile, index: number, custom?: () => ReactNode) => (
            <div className={utils.getItemClass(file)} key={file.id}>
                <div className="pl-upload-item-inner">
                    {!!custom ? custom() : <>
                        {file.status ? renderIcon[file.status] : <PlIcon icon="el-icon-document"/>}
                        {file.status === UploadStatus.ready ? '(待上传) ' : ''}
                        <div className="pl-upload-item-content" onClick={() => !!props.handlePreview && props.handlePreview(file)}>
                    ...
                    {(
                        ...
                        <div className="pl-upload-item-remove" onClick={() => methods.removeFile(file)}>
                            <PlIcon icon="el-icon-delete-solid"/>
                        </div>
                    )}
                </div>
                {file.percent != null && (
                    <div className="pl-upload-item-progress">
                        <div className="pl-upload-item-progress-inner" style={{width: `${file.percent}%`}}/>
                    </div>
                )}
            </div>
        )
        ...
        const multipleRender = computed(() => <>
            <div className="pl-upload-button">
                {!props.draggable ? (
                    <PlButtonGroup>
                        <PlButton label="选择文件" icon="el-icon-upload" onClick={methods.chooseFile}/>
                        {!props.autoUpload && <PlButton label="上传" icon="el-icon-connection" onClick={methods.upload}/>}
                    ...
                    <div className="pl-upload-drop-area" onClick={methods.chooseFile} {...dropHandler}>
                        <PlIcon icon="el-icon-upload"/>
                        <div>
                            <span>将文件拖拽至此处，或者</span>
                            <PlButton mode="text" className="pl-upload-drop-upload-button" onClick={methods.chooseFile}>
                                <span>点击上传</span>
