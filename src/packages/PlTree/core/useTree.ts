...
import {computed, PropType, useModel} from "plain-design-composition";
...
function use<Node extends {
    ...
    const handler = {
        /*点击展开图标*/
        onClickExpandIcon: async (e: React.MouseEvent, node: Node) => {
            e.stopPropagation()
            await expandMethods.toggleExpand(node)
        },
        /*点击复选框*/
        onClickCheckbox: async (e?: React.MouseEvent, node?: Node) => {
            !!e && e.stopPropagation();
            !!node && await checkMethods.toggleCheck(node)
        },
        /*点击行*/
        onClickCell: async (e: React.MouseEvent, node: Node) => {
